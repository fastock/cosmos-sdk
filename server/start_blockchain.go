...
// blockchain full-node start flags
const (
...
func StopCmd(ctx *Context) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				errStr := fmt.Sprintf("%s Please finish the process of fastock-chain-daemon through kill -2 pid to stop gracefully", err.Error())
				cmn.Exit(errStr)
			}
			defer f.Close()
...
func registerRestServerFlags(cmd *cobra.Command) *cobra.Command {
	...
	cmd.Flags().String(FlagRestPathPrefix, "blockchain", "Path prefix for registering rest api route.")
...
// registerblockchainPluginFlags registers the flags required for rest server
func registerblockchainPluginFlags(cmd *cobra.Command) *cobra.Command {
